window.onload = function() {
    ...
    function calculateInterpolation() {
      ...
      if (isNaN(a) || isNaN(b) || isNaN(n) || a >= b || n <= 0 || !Number.isInteger(n)) {
        alert("Введіть правильні числові значення для a, b, та n! (a < b, n - ціле додатнє число)");
        return;
      }
      ...
      const fValues = nodes.map(cosOfSquare);
      ...
      drawChart("function-chart", "cos(x^2)", nodes, fValues, "red");
    ...
    // Функція для обчислення значення інтерполяційного многочлена Ньютона (використовується алгоритм Нестеда)
